...
//   const outStockCount = products.filter(p => p.stockStatus === 'Out of Stock').length;

//   const departments = useMemo(() => {
//     const set = new Set(products.map(p => p.departmentName).filter(Boolean));
...
//     });
//   }, [products, search, statusFilter, stockFilter, deptFilter]);
...
//         // Summary row — first cell has rowSpan; remaining 3 cells fill the rest.
//         // NOTE: no placeholder cell — autoTable auto-fills the rowSpan-covered slot.
//         body.push([
//           {
//             content: dept,
...
//         // Detail rows — ONLY 3 cells (Department column is covered by rowSpan)
//         items.forEach(item => {
//           body.push([
//             {
...
//         columnStyles: {
//           0: { cellWidth: 160, halign: 'left' },    // Department
//           1: { cellWidth: 'auto', halign: 'left' }, // Product Name
//           2: { cellWidth: 100, halign: 'center' },  // Quantity
...
//       doc.text('Product Inventory & Request Report', marginLeft, pageHeight - 20);
...
//             <p style={{ color: '#64748b' }}>No approved or rejected requests yet.</p>
//           ) : (
...
const ProductReport = () => {
  ...
  // Date field used for filtering: prefer receiptDate, fallback issueDate
  const getFilterDate = (item) => item.receiptDate || item.issueDate || null;

  // Determine year list from data (fallback to current year)
  const years = useMemo(() => {
    const set = new Set();
    products.forEach(p => {
  ...
  // Base set filtered only by year/quarter — used for department usage
  ...
  // Fully filtered products (period + user filters)
  const filteredProducts = useMemo(() => {
  ...
  // Department-wise usage — respects year/quarter
  useEffect(() => {
    const deptDetails = {};
    requests.forEach(req => {
      if (req.status !== 'APPROVED' && req.status !== 'REJECTED') return;

      // Use receiptDate / issueDate of the linked product as the period date
