...
const AdminRequestProduct = () => {
  ...
  const requestType = 'STOCK_REQUEST';
  const [productId, setProductId] = useState('');
  const [quantity, setQuantity] = useState('');
  const [issueDate, setIssueDate] = useState('');
  const [description, setDescription] = useState('');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccess('Stock request sent to Super Admin!');
        ...
        {!loadingData && (
          <form onSubmit={handleSubmit}>
            ...
            <div style={row}>
              ...
              <div style={col}>
                ...
                <textarea
                  ...
                  rows={2}
                  style={{ ...inputStyle, height: 48, padding: '12px 14px', resize: 'vertical' }}
                  placeholder="Explain why you need more stock (e.g. stock finished)"
                />
              </div>
            </div>
            ...
            <div style={{ marginTop: 28 }}>
              ...
              >
                {loading ? 'Sending...' : 'Send Stock Request →'}
              </button>
            </div>
          </form>
        )}
