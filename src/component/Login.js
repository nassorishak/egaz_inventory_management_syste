  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (role === 'STAFF') {
        ...
        navigate('/supper-admin-dashboard');   // ✅ FIX: imebadilishwa
      } else {
        setError(`Access denied: invalid role (${role}).`);
      }
    } catch (err) {
      console.error('Login error:', err);

      if (err.message && err.message.includes('431')) {
        setError('Request header too large — clear localStorage and cookies.');
      } else if (err.code === 'ERR_NETWORK') {
    ...
    <div style={styles.container}>
      ...
      <div style={styles.card}>
        ...
        <div style={styles.rightSection}>
          ...
          <Link to="/registration" style={{ textDecoration: 'none', display: 'block' }}>
            <button
              type="button"
              style={styles.secondaryButton}
              onMouseOver={(e) => {
          ...
          <p style={styles.footerText}>
            Staff & Admin & Supper-Admin Portal
          </p>
        </div>
      </div>
    </div>
...
const styles = {
  ...
  title: { margin: '0 0 8px 0', color: '#111827', fontSize: '32px', fontWeight: '750' },
  ...
  arrow: { fontSize: '20px', lineHeight: 1 },
  spinner: {
    width: '16px',
    height: '16px',
    border: '2px solid rgba(255,255,255,0.4)',
  ...
  dividerText: { color: '#9ca3af', fontSize: '11px', fontWeight: '600' },
  secondaryButton: {
    width: '100%',
    height: '50px',
    borderRadius: '10px',
